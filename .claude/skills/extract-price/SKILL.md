...
2. Выполни на странице скрипт `extract.js` из корня проекта через `javascript_tool`, передав содержимое файла целиком. Скрипт сам ждёт догрузки страницы и возвращает JSON в формате выше
