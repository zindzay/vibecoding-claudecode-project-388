...
- `KNOWLEDGE.md` (в корне проекта) — правила значимости изменения цены
- `send.py` (в корне проекта) — отправка текста в Telegram
...
3. Для каждого URL получи цену по правилам скилла `extract-price`: открой страницу в браузере и выполни `extract.js` из корня проекта через `javascript_tool`. Логику разбора не дублируй — используй этот скрипт. Скрипт ждёт несколько секунд после загрузки — это и есть пауза между товарами против антибота Ozon. Удобно обходить товары пачками через `browser_batch` (navigate + javascript_tool на каждый товар)
4. Собери таблицу в формате снимка, включая поле `status` из extract-price (`ok` или `out_of_stock`). У товара не в продаже `regular_price` и `sale_price` равны `null`. В старых снимках `status` может отсутствовать — при сравнении ориентируйся на цены, а не на `status`
...
6. **Анализ**:
   ...
   - примени правила из `KNOWLEDGE.md` в корне проекта
...
9. **Сводка — модель Haiku.** Если diff непустой, запусти сабагента `tg-summary` (`.claude/agents/tg-summary.md`, работает на Haiku) через Agent tool с `subagent_type: "tg-summary"` и передай ему только дату снимка и готовый diff из шага 6. Формат текста описан в самом агенте, здесь его не дублируй
10. Отправка:
    - diff непустой — напиши сводку в чат и отправь её в Telegram из корня проекта: `SKILL_ENV_FILE=<путь к .env> python3 send.py "<текст>"`
